...
public class BaseAuthorPage extends BasePage implements AuthorPage {
	...
	// New UI
	@FindBy(xpath = "//div[@id='Content']/div/nav/div/button[@title='Toggle Side Panel']")
	...
	public void fillInDialogFieldByName(String name, String text) {
		...
		try {
			while (!inputString.equals(text) && retries < maxRetries) {
				...
				wait.until(ExpectedConditions.elementToBeClickable(fieldBy));
				...
				retries++;
				Thread.sleep(500);
				inputString = getTextFromInput(fieldBy);
			}
